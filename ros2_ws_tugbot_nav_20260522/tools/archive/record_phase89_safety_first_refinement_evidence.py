#!/usr/bin/env python3
"""Phase89 raw evidence recorder for bounded Goal2 validation.
...
the bounded Phase88 refinement validation scene. It sends no goals and changes
...
class Phase89Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase89_safety_first_refinement_raw_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase89Recorder(args.duration_sec, source)

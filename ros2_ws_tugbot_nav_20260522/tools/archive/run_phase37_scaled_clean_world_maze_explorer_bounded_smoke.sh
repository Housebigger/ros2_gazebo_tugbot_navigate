...
RUN_TIMEOUT_SEC="${PHASE37_RUN_TIMEOUT_SEC:-900}"
MAX_GOALS="${PHASE37_MAX_GOALS:-4}"
GOAL_TIMEOUT_SEC="${PHASE37_GOAL_TIMEOUT_SEC:-45.0}"
SNAPSHOT_DELAY_SEC="${PHASE37_SNAPSHOT_DELAY_SEC:-45}"
STATE_RECORDER_TIMEOUT_SEC="${PHASE37_STATE_RECORDER_TIMEOUT_SEC:-930}"
USE_RVIZ="${PHASE37_USE_RVIZ:-false}"
...
cleanup() {
  ...
  pkill -TERM -f 'tools/record_explorer_state_series.py.*phase37_scaled_clean_world_maze_explorer_bounded_smoke' || true
...
{
  echo "# Phase37 scaled clean world maze_explorer bounded autonomous smoke"
  ...
  echo "active_truth=entrance_x=0.0 entrance_y=0.0 entrance_yaw=0.0 exit_x=21.072562 exit_y=18.083566 exit_radius=1.2"
...
echo "Phase37 bounded smoke complete. Artifacts: $ARTIFACT_DIR"

...
REPLAY_COUNT="${PHASE70_REPLAY_COUNT:-2}"
MAX_GOALS="${PHASE70_MAX_GOALS:-1}"
if (( MAX_GOALS != 1 )); then
  echo "PHASE70_MAX_GOALS must be exactly 1; got $MAX_GOALS" >&2
  exit 2
fi
if (( REPLAY_COUNT < 1 || REPLAY_COUNT > 3 )); then
  echo "PHASE70_REPLAY_COUNT must be 1..3; got $REPLAY_COUNT" >&2
  exit 2
fi
...
ARTIFACT_DIR="${PHASE70_ARTIFACT_DIR:-$ROOT/log/${RUN_ID}}"
...
RUN_TIMEOUT_SEC="${PHASE70_RUN_TIMEOUT_SEC:-240}"
READINESS_TIMEOUT_SEC="${PHASE70_READINESS_TIMEOUT_SEC:-80}"
POST_READINESS_SETTLE_SEC="${PHASE70_POST_READINESS_SETTLE_SEC:-10}"
GOAL_TIMEOUT_SEC="${PHASE70_GOAL_TIMEOUT_SEC:-100}"
EXPLORER_OBSERVE_SEC="${PHASE70_EXPLORER_OBSERVE_SEC:-120}"
RUNTIME_RECORD_TIMEOUT_SEC="${PHASE70_RUNTIME_RECORD_TIMEOUT_SEC:-145}"
TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES="${PHASE70_TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES:-2}"
TOPOLOGY_CONSISTENCY_WINDOW_SEC="${PHASE70_TOPOLOGY_CONSISTENCY_WINDOW_SEC:-4.0}"
PIDS=()
LAUNCH_PID=""
STATE_PID=""
GOAL_EVENTS_PID=""
...
cleanup_processes_after() {
  ...
  pkill -TERM -f '[a]nalyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation.py --send-inner-ingress-goal' || true
  pkill -TERM -f '[a]nalyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation.py --record-runtime' || true
  pkill -TERM -f '[r]ecord_explorer_state_series.py.*phase70_centerline_gate_relaxation_balance_first_runtime_validation' || true
  ...
  (pgrep -af '[r]os2 launch|[r]os2 run tugbot_maze maze_explorer|[g]z sim|[r]viz2|[s]lam_toolbox|[m]aze_explorer|[m]aze_goal_monitor|[f]rontier_explorer|[c]ontroller_server|[p]lanner_server|[b]t_navigator|[b]ehavior_server|[w]aypoint_follower|[v]elocity_smoother|[s]moother_server|[r]oute_server|[c]ollision_monitor|[d]ocking_server|[r]os_gz_bridge|[p]arameter_bridge|[s]tatic_transform_publisher|[a]nalyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation|[r]ecord_explorer_state_series.py.*phase70_centerline_gate_relaxation_balance_first_runtime_validation' || true) \
    | grep -v 'hermes-snap' \
    | grep -v 'pgrep -af' \
    > "$CLEANUP_AFTER" || true
}
...

{
  ...
  echo "phase69_classification=CENTERLINE_GATE_NO_APPLY"
  ...
  echo "guardrails=bounded runtime only; max_goals=1; no Nav2/MPPI/controller parameter edits; no inflation/robot_radius/clearance_radius_m/map threshold tuning; no branch scoring change; no corridor-following cmd_vel control; no fallback/terminal acceptance change; no old scaffold world/map; no autonomous exploration success claim; no exit success claim."
  echo "maze_explorer_command=ros2 run tugbot_maze maze_explorer --ros-args -p max_goals:=$MAX_GOALS -p near_exit_fallback_enabled:=false -p centerline_target_refinement_enabled:=true -p centerline_target_refinement_gate_mode:=balance_first"
...
start_runtime_recorder_after_ingress() {
  local replay_dir="$1"
  local replay_id="${replay_dir##*/}"
  python3 tools/analyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation.py --record-runtime \
    --timeout-sec "$RUNTIME_RECORD_TIMEOUT_SEC" \
    --output "$replay_dir/${RUN_ID}_${replay_id}_runtime_recorder_summary.json" \
    --timeline-output "$replay_dir/phase70_runtime_timeline.jsonl" \
    --controller-dynamics-output "$replay_dir/phase70_controller_dynamics.jsonl" \
    --nav2-feedback-output "$replay_dir/phase70_nav2_feedback.jsonl" \
    --local-costmap-samples-output "$replay_dir/phase70_local_costmap_samples.jsonl" \
    --global-plan-samples-output "$replay_dir/phase70_global_plan_samples.jsonl" \
    --collision-monitor-output "$replay_dir/phase70_collision_monitor_state.jsonl" \
...
for replay_index in $(seq 1 "$REPLAY_COUNT"); do
  ...
  echo "phase70 replay $replay_index/$REPLAY_COUNT starting" >&2
...
JSON
    ...
    python3 tools/analyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation.py --send-inner-ingress-goal \
...
PY
  ...
  trap cleanup_processes_after EXIT
  sleep 5
  echo "phase70 replay $replay_index/$REPLAY_COUNT completed" >&2
done

cleanup_processes_after
trap - EXIT

python3 tools/analyze_phase70_centerline_gate_relaxation_balance_first_runtime_validation.py --analyze \

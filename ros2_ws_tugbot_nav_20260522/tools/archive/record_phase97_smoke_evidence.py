#!/usr/bin/env python3
"""Phase97 raw evidence recorder.

Read-only recorder for ingress-guided Phase97 multi-goal smoke. Captures
/maze/goal_events, /scan, /map, /local_costmap/costmap,
/local_costmap/published_footprint, /odom, and TF. It creates no action client,
sends no goals, and changes no parameters.
"""
...
def occupancy_to_dict(msg: OccupancyGrid | None, topic: str) -> dict[str, Any] | None:
    if msg is None:
        return None
    ...
    return {
        ...
        'summary': {
            ...
            'free_count': sum(1 for v in values if v == 0),
            'occupied_count': sum(1 for v in values if v >= 50),
...
class Phase97Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase97_smoke_evidence_recorder')
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            'map': occupancy_to_dict(self.map_msg, '/map'),
            'local_costmap': occupancy_to_dict(self.costmap_msg, '/local_costmap/costmap'),
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase97Recorder(args.duration_sec, load_json(args.analysis_json))
    try:
        ...
        print(json.dumps({'output': str(args.output), 'scan': snapshot['scan'] is not None, 'map': snapshot['map'] is not None, 'local_costmap': snapshot['local_costmap'] is not None, 'odom': snapshot['odom'] is not None, 'tf_keys': sorted(snapshot['tf'])}, sort_keys=True))
    finally:
        node.destroy_node()
        rclpy.shutdown()
    return 0

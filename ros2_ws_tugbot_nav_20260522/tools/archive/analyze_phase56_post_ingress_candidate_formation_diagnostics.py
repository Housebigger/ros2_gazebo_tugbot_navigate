#!/usr/bin/env python3
"""Phase56 Post-Ingress Open Direction to Candidate Formation Diagnostics recorder/analyzer.
...
Phase56 diagnoses why a post-ingress raw open direction does or does not
become a dispatchable branch candidate. Artifact contract: ingress action result;
explorer_state.jsonl; goal_events.jsonl; first topology sampling snapshot;
open_direction_to_candidate_diagnostics.json; map/scan/TF/local/global costmap evidence;
summary/analysis JSON; cleanup_processes_after.txt. If dispatch appears, this
means candidate formation restored only; first dispatch is not exit success. It
never claims autonomous exploration success and does not require reaching the exit.
"""
...
PHASE = 'Phase56 Post-Ingress Open Direction to Candidate Formation Diagnostics'
...
class RuntimeRecorder(Node):  # pragma: no cover - requires ROS graph
    def __init__(self, duration_sec: float, snapshot_period_sec: float, output: Path):
        super().__init__('phase56_post_ingress_candidate_runtime_recorder')
...
class IngressGoalClient(Node):  # pragma: no cover - requires ROS graph
    def __init__(self, output: Path, goal_timeout_sec: float):
        super().__init__('phase56_ingress_goal_client')
        self.output = output
...
def _topology_rejection_summary(topology: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        'candidate_before_filter_count': topology.get('candidate_before_filter_count'),
        'candidate_after_filter_count': topology.get('candidate_after_filter_count'),
        'reason_counts': counts,
    }


def extract_open_direction_to_candidate_diagnostics(first_topology_state: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(first_topology_state, dict):
        return {'available': False, 'rejection_step': 'no_topology_state'}
    topology = first_topology_state.get('last_topology_sampling_diagnostics')
    if not isinstance(topology, dict):
        return {'available': False, 'rejection_step': 'no_topology_diagnostics'}
    ...
    if not isinstance(phase56, dict):
        phase56 = {'available': False, 'rejection_step': 'phase56_diagnostics_missing'}
    out = dict(phase56)
    out['available'] = bool(phase56.get('available', True))
    out['raw_open_direction_count'] = topology.get('raw_open_direction_count')
    out['filtered_open_direction_count'] = topology.get('filtered_open_direction_count')
    out['candidate_before_filter_count'] = topology.get('candidate_before_filter_count')
    out['candidate_after_filter_count'] = topology.get('candidate_after_filter_count')
    out['candidate_branch_count'] = topology.get('candidate_branch_count')
    out['reject_reason_counts'] = topology.get('reject_reason_counts')
    out['first_topology_sampling_snapshot'] = topology
    return out


def classify_candidate_formation(
    *,
    guardrail_violation: bool,
    first_dispatch_event: dict[str, Any] | None,
    diagnostics: dict[str, Any],
    first_topology_state: dict[str, Any] | None,
) -> str:
    if guardrail_violation:
        return 'GUARDRAIL_VIOLATION_STRATEGY_CHANGED'
    if first_dispatch_event is not None:
        return 'CANDIDATE_FORMATION_REJECTION_CAUSE_IDENTIFIED'
    reason = str(diagnostics.get('branch_candidate_rejection_reason') or diagnostics.get('rejection_step') or '')
    if reason in {'filtered_by_min_travel_distance', 'raw_branch_goal_below_min_step', 'too_close_to_robot'}:
        return 'CANDIDATE_REJECTED_BY_MIN_DISTANCE'
    if reason in {'raw_branch_goal_clearance_failed', 'centered_branch_goal_unavailable', 'filtered_by_candidate_clearance', 'candidate_clearance_blocked'}:
        return 'CANDIDATE_REJECTED_BY_CLEARANCE'
    if reason in {'duplicate_or_exhausted', 'duplicate_branch', 'exhausted_branch', 'blacklisted'}:
        return 'CANDIDATE_REJECTED_AS_DUPLICATE_OR_EXHAUSTED'
    if reason in {'dead_end_policy_no_branch_options', 'junction_or_dead_end_policy', 'dead_end_no_backtrack', 'node_policy_no_untried_branch'}:
        return 'CANDIDATE_REJECTED_BY_JUNCTION_OR_DEAD_END_POLICY'
    if isinstance(first_topology_state, dict) and diagnostics.get('available'):
        return 'CANDIDATE_FORMATION_REJECTION_CAUSE_IDENTIFIED'
    return 'OPEN_DIRECTION_TO_CANDIDATE_INCONCLUSIVE'


def write_artifact_json(path: Path | None, data: dict[str, Any]) -> None:
    if path is None:
        return
...
def analyze(args: argparse.Namespace) -> int:
    ...
    action_status_succeeded = action.get('status_text') == 'STATUS_SUCCEEDED' or int(action.get('error_code') or -1) == 0
    # Phase56 is post-ingress: after the ingress action succeeds, maze_explorer may
    # legitimately dispatch a bounded max_goals=1 candidate and move away from the
    # ingress waypoint. Therefore final runtime distance-to-ingress is evidence, but
    # must not be used to retroactively mark the ingress handoff as failed.
    ...
    ingress_reached = bool(action_success and action_result_received and action_status_succeeded)
    ...
    forbidden_unbounded = 'GUARDRAIL_VIOLATION_STRATEGY_CHANGED' in launch_text or 'complete autonomous success' in launch_text.lower()
    cleanup_empty = cleanup_text.strip() == ''
    open_direction_to_candidate_diagnostics = extract_open_direction_to_candidate_diagnostics(first_topology_state)
    guardrail_violation = bool(
        forbidden_unbounded
        or goal_count_violation
        or not max_goals_limited_to_one
        or not near_exit_fallback_disabled
        or not startup_warmup_no_dispatch_false
    )

    if not ingress_reached:
        classification = 'OPEN_DIRECTION_TO_CANDIDATE_INCONCLUSIVE'
    else:
        classification = classify_candidate_formation(
            guardrail_violation=guardrail_violation,
            first_dispatch_event=first_dispatch_event,
            diagnostics=open_direction_to_candidate_diagnostics,
    ...
    summary = {
        ...
        'ingress_action_status_succeeded': bool(action_status_succeeded),
        ...
        'first_topology_sampling_snapshot': topology_sampling_diagnostics,
        'open_direction_to_candidate_diagnostics': open_direction_to_candidate_diagnostics,
        ...
        'candidate_formation_restored': bool(first_dispatch_event is not None),
        ...
        'report_scope': [
            'Phase56 diagnoses post-ingress open direction to candidate formation only.',
            'It does not claim autonomous exploration success.',
            'It does not require reaching the exit.',
            'If a first dispatch appears, it means candidate formation restored only; first dispatch is not exit success.',
        ],
    }
    write_artifact_json(Path(args.first_topology_snapshot) if args.first_topology_snapshot else None, topology_sampling_diagnostics or {})
    write_artifact_json(Path(args.open_direction_diagnostics) if args.open_direction_diagnostics else None, open_direction_to_candidate_diagnostics)
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--artifact-dir', default='log/phase56_post_ingress_candidate_formation_diagnostics')
    ...
    parser.add_argument('--first-topology-snapshot')
    parser.add_argument('--open-direction-diagnostics')

...
# Phase102: Phase101 carry-over bounded Goal1 staging validation.
# Validation-only. Visible Gazebo/RViz/SLAM/Nav2, explicit inner-ingress Nav2
# goal before maze_explorer, then bounded Goal1 observation until carry-over /
# staging context, Goal1 terminal outcome, or bounded failure is observed.
# No maze_explorer strategy, Phase88/92/101 logic, branch scoring, exploration
# order, centerline gate, directional readiness, fallback/terminal acceptance,
# Nav2/MPPI/controller/inflation/robot_radius/clearance_radius_m/map threshold tuning.

RUN_ID="phase102_carry_over_bounded_goal1_staging_validation"
PHASE="Phase102 Phase101 carry-over bounded Goal1 staging validation"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

MAX_GOALS="${PHASE102_MAX_GOALS:-1}"
if (( MAX_GOALS != 1 )); then
  echo "PHASE102_MAX_GOALS must be 1: bounded Goal1 validation only" >&2
  exit 2
fi
...
PHASE102_INGRESS_X="${PHASE102_INGRESS_X:-2.0}"
PHASE102_INGRESS_Y="${PHASE102_INGRESS_Y:-0.0}"
PHASE102_INGRESS_YAW="${PHASE102_INGRESS_YAW:-0.0}"
INGRESS_TIMEOUT_SEC="${PHASE102_INGRESS_TIMEOUT_SEC:-90}"
GOAL_TIMEOUT_SEC="${PHASE102_GOAL_TIMEOUT_SEC:-190.0}"
READINESS_TIMEOUT_SEC="${PHASE102_READINESS_TIMEOUT_SEC:-150}"
POST_READINESS_SETTLE_SEC="${PHASE102_POST_READINESS_SETTLE_SEC:-5}"
VALIDATION_WINDOW_SEC="${PHASE102_VALIDATION_WINDOW_SEC:-260}"
RUNTIME_RECORD_TIMEOUT_SEC="${PHASE102_RUNTIME_RECORD_TIMEOUT_SEC:-320}"
RAW_CAPTURE_DURATION_SEC="${PHASE102_RAW_CAPTURE_DURATION_SEC:-5}"
INGRESS_PREFLIGHT_TIMEOUT_SEC="${PHASE102_INGRESS_PREFLIGHT_TIMEOUT_SEC:-20}"
INGRESS_PREFLIGHT_TF_STABILITY_WINDOW_SEC="${PHASE102_INGRESS_PREFLIGHT_TF_STABILITY_WINDOW_SEC:-2.0}"
INGRESS_PREFLIGHT_SAMPLE_PERIOD_SEC="${PHASE102_INGRESS_PREFLIGHT_SAMPLE_PERIOD_SEC:-0.5}"
INGRESS_PREFLIGHT_TF_MAX_AGE_SEC="${PHASE102_INGRESS_PREFLIGHT_TF_MAX_AGE_SEC:-1.5}"
INGRESS_PREFLIGHT_SCAN_MAX_AGE_SEC="${PHASE102_INGRESS_PREFLIGHT_SCAN_MAX_AGE_SEC:-1.5}"
CLEANUP_ON_EXIT="${PHASE102_CLEANUP_ON_EXIT:-0}"
REUSE_VISIBLE_STACK="${PHASE102_REUSE_VISIBLE_STACK:-0}"
SKIP_BUILD="${PHASE102_SKIP_BUILD:-0}"
...
wait_for_readiness() {
  ...
  while (( $(date +%s) - start < READINESS_TIMEOUT_SEC )); do
    if ros2 action list 2>/dev/null | grep -q '^/navigate_to_pose$' \
      ...
      && timeout 8 bash -c 'ros2 run tf2_ros tf2_echo map base_link 2>&1 | tee /tmp/phase102_tf_ready.txt | grep -m 1 "Translation:" >/dev/null'; then
      {
        ...
        cat /tmp/phase102_tf_ready.txt || true
        ros2 node list || true
...
wait_for_ingress_terminal() {
  python3 - "$INGRESS_RESULT" "$PHASE102_INGRESS_X" "$PHASE102_INGRESS_Y" "$PHASE102_INGRESS_YAW" <<'PY'
import json, sys
from pathlib import Path
...
run_analyzer() {
  python3 tools/analyze_phase102_carry_over_bounded_goal1_staging_validation.py \
...

{
  ...
  echo "inner_ingress_goal=(${PHASE102_INGRESS_X},${PHASE102_INGRESS_Y},${PHASE102_INGRESS_YAW})"
  echo "max_goals=$MAX_GOALS"
  echo "goal_timeout_sec=$GOAL_TIMEOUT_SEC"
  echo "validation_window_sec=$VALIDATION_WINDOW_SEC"
  echo "ingress_preflight=fail-closed TF/controller preflight before sender"
  ...
  echo "wait_for_goal1_carry_over_or_terminal"
  echo "No Phase88/92/101 logic changed"
...
python3 tools/record_phase102_carry_over_validation_evidence.py \

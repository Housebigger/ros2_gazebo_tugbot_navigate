#!/usr/bin/env python3
"""Phase97 ingress-guided refinement-chain bounded multi-goal smoke analyzer.

Read-only analyzer for a correctly started Phase88/92 refinement-chain smoke:
visible stack -> explicit inner-ingress Nav2 goal -> maze_explorer max_goals 2..3.
It classifies terminal multi-goal stability without changing algorithm, Nav2,
branch scoring, exploration order, centerline gate, directional readiness,
fallback, or terminal acceptance.
"""
...
def _extract_multi_candidate(row: JSONDict | None) -> JSONDict:
    ...
    if isinstance(nested, dict):
        return nested
    nested = row.get('multi_candidate_forward_search')
...
def _feedback_summary(feedback: list[JSONDict], seq: int) -> JSONDict:
    ...
    for row in rows:
        ...
        if last is None or rec != last:
            timeline.append({'elapsed_sec': row.get('elapsed_sec') or row.get('_recorder_elapsed_sec'), 'recoveries': rec})
            last = rec
    first_nonzero = next((v for v in distances if v > 0.0), None)
    return {
...
def _goal_summary(seq: int, rows: list[JSONDict], feedback: list[JSONDict], local_samples: list[JSONDict]) -> JSONDict:
    dispatch = next((row for row in rows if str(row.get('event') or '').lower() == 'dispatch'), rows[0] if rows else {})
    ...
    staging_applied = _boolish(dispatch.get('staging_applied')) or _boolish(two_step.get('staging_applied')) or _boolish(two_step.get('enabled') and dispatch.get('goal_kind') == 'corridor_alignment_staging')
    ...
    return {
        ...
        'dispatch': dispatch,
        'dispatch_observed': str(dispatch.get('event') or '').lower() == 'dispatch',
        'goal_kind': dispatch.get('goal_kind'),
        'target': dispatch.get('target') or dispatch.get('nav2_goal_target'),
        'selected_candidate_target': ref.get('selected_candidate_target') or multi.get('selected_candidate_target') or dispatch.get('target'),
        'selected_candidate_yaw': ref.get('selected_candidate_yaw') or multi.get('selected_candidate_yaw') or dispatch.get('branch_angle'),
        ...
        'multi_candidate_forward_search': {
            'present': bool(multi),
            'candidate_count': multi.get('candidate_count') or ref.get('centerline_refinement_candidate_count') or dispatch.get('centerline_refinement_candidate_count'),
            'candidate_family': multi.get('candidate_family') or ref.get('candidate_family'),
            'selected_candidate_index': multi.get('selected_candidate_index') or ref.get('selected_candidate_index'),
        ...
        'two_step_staging_plan': {
            ...
            'trigger_conditions': two_step.get('trigger_conditions'),
            'source_single_step': two_step.get('source_single_step'),
        },
        ...
        'terminal_reason': terminal_row.get('reason') or terminal_row.get('terminal_reason') if terminal_row else None,
...
def _has_capture_key(raw_capture: JSONDict, key: str) -> bool:
    return isinstance(raw_capture, dict) and key in raw_capture and raw_capture.get(key) is not None
...
def _required_evidence_gaps(feedback: list[JSONDict], local_samples: list[JSONDict], raw_capture: JSONDict) -> list[str]:
    ...
    if not raw_capture:
        gaps.append('raw_scan_map_odom_tf_capture_missing')
    else:
        for key in ['scan', 'map', 'local_costmap', 'odom', 'tf']:
            if not _has_capture_key(raw_capture, key):
                gaps.append(f'{key}_capture_missing')
    return gaps


def _classify(per_goal: dict[str, JSONDict], evidence_gaps: list[str], observed_goal_count: int, ingress_ok: bool) -> str:
    if not ingress_ok:
        return 'INGRESS_GUIDED_MULTI_GOAL_INGRESS_FAILED'
    if evidence_gaps or observed_goal_count == 0:
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_INSUFFICIENT_EVIDENCE'
    goals = list(per_goal.values())
    if any(goal.get('staging_applied') for goal in goals):
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_STAGING_TRIGGERED_NEEDS_REVIEW'
    if any((goal.get('recoveries') or {}).get('max', 0) >= RECOVERY_DOMINANT_THRESHOLD for goal in goals):
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_RECOVERY_DOMINANT'
    if any(goal.get('terminal_outcome') in {'timeout', 'failure', 'cancelled'} for goal in goals):
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_GOAL_TIMEOUT'
    if observed_goal_count < 2:
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_INSUFFICIENT_EVIDENCE'
    if any(goal.get('terminal_outcome') is None for goal in goals):
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_INSUFFICIENT_EVIDENCE'
    if all(goal.get('terminal_outcome') == 'succeeded' for goal in goals):
        return 'INGRESS_GUIDED_REFINEMENT_CHAIN_BOUNDED_PASS'
    return 'INGRESS_GUIDED_REFINEMENT_CHAIN_INSUFFICIENT_EVIDENCE'
...
def analyze_artifacts(artifact_dir: str | Path, *, run_id: str = RUN_ID, max_goals: int = 3) -> JSONDict:
    ...
    evidence_gaps = [] if not ingress or not _ingress_success(ingress) else _required_evidence_gaps(feedback, local_samples, raw_capture)
    ingress_ok = _ingress_success(ingress)
    classification = _classify(per_goal, evidence_gaps, len(selected_sequences), ingress_ok)
    return {
        'run_id': run_id,
        ...
        'ingress': {
            'success': ingress_ok,
            'status': ingress.get('status') or ingress.get('status_text'),
            'target': ingress.get('target') or ingress.get('goal_pose') or ingress.get('inner_ingress_waypoint_map'),
            'feedback_sample_count': len(ingress.get('feedback') or []) if isinstance(ingress, dict) else 0,
            'raw': ingress,
        },
        ...
        'stream_counts': {
            ...
            'nav2_feedback': len(feedback),
            'local_costmap_samples': len(local_samples),
        },
        'raw_capture_summary': {
            'available': bool(raw_capture),
            'scan_available': _has_capture_key(raw_capture, 'scan'),
            'map_available': _has_capture_key(raw_capture, 'map'),
            'local_costmap_available': _has_capture_key(raw_capture, 'local_costmap'),
            'odom_available': _has_capture_key(raw_capture, 'odom'),
            'tf_available': _has_capture_key(raw_capture, 'tf'),
        },
        'guardrails': {
...
def _write_minimal_summary(result: JSONDict, path: Path) -> None:
    lines = [
        '# Phase97 ingress-guided refinement chain minimal field summary',
        '',
        f"Classification: {result.get('classification')}",
        f"Run/artifacts: {result.get('artifact_dir')}",
        f"Ingress success: {result.get('ingress', {}).get('success')}",
    ...
    for seq, goal in result.get('per_goal', {}).items():
        lines.extend([
            f"## Goal {seq}",
            f"dispatch_observed: {goal.get('dispatch_observed')}",
    ...
    lines.extend([
        'No autonomous exploration success claimed. No exit success claimed. Phase98 not entered.',
        '',
    ])

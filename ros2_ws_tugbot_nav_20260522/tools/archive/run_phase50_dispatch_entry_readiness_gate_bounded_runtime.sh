...
RUN_TIMEOUT_SEC="${PHASE50_RUN_TIMEOUT_SEC:-180}"
MAX_GOALS="${PHASE50_MAX_GOALS:-1}"
GOAL_TIMEOUT_SEC="${PHASE50_GOAL_TIMEOUT_SEC:-45.0}"
SNAPSHOT_DELAY_SEC="${PHASE50_SNAPSHOT_DELAY_SEC:-60}"
STATE_RECORDER_TIMEOUT_SEC="${PHASE50_STATE_RECORDER_TIMEOUT_SEC:-210}"
RUNTIME_RECORDER_TIMEOUT_SEC="${PHASE50_RUNTIME_RECORDER_TIMEOUT_SEC:-150}"
USE_RVIZ="${PHASE50_USE_RVIZ:-false}"

if (( RUN_TIMEOUT_SEC < 120 || RUN_TIMEOUT_SEC > 240 )); then
  echo "Phase50 RUN_TIMEOUT_SEC must be bounded in [120, 240], got $RUN_TIMEOUT_SEC" >&2
  exit 2
fi
...
RUNTIME_EVIDENCE="$ARTIFACT_DIR/phase50_runtime_evidence.json"
SUMMARY_JSON="$ARTIFACT_DIR/phase50_dispatch_entry_readiness_gate_bounded_runtime.json"
...
cleanup() {
  ...
  pkill -TERM -f 'tools/record_explorer_state_series.py.*phase50_dispatch_entry_readiness_gate_bounded_runtime' || true
  pkill -TERM -f 'tools/analyze_phase50_dispatch_entry_readiness_gate_runtime.py --record-runtime' || true
  ...
  pgrep -af 'ros2 launch|gz sim|rviz2|slam_toolbox|maze_explorer|frontier_explorer|controller_server|planner_server|bt_navigator|behavior_server|waypoint_follower|velocity_smoother|smoother_server|route_server|collision_monitor|docking_server|ros_gz_bridge|parameter_bridge|static_transform_publisher|maze_goal_monitor|analyze_phase50_dispatch_entry_readiness_gate_runtime.py|record_explorer_state_series.py' > "$CLEANUP_AFTER" || true
}
trap cleanup EXIT
...
rm -f \
  ...
  "$ARTIFACT_DIR/${RUN_ID}_ros_graph_snapshot.txt" \
  "$ARTIFACT_DIR/${RUN_ID}_gz_graph_snapshot.txt" \
...
{
  echo "# Phase50 Dispatch Entry Readiness Gate Bounded Runtime Validation"
  ...
  echo "guardrails=bounded runtime validation only; no clearance_radius_m tuning; no Nav2/MPPI/controller parameter edits; no maze_explorer strategy edits; no fallback/terminal acceptance continuation; no autonomous success claim."
...
python3 tools/analyze_phase50_dispatch_entry_readiness_gate_runtime.py --record-runtime "$RUNTIME_RECORDER_TIMEOUT_SEC" \
...
python3 tools/analyze_phase50_dispatch_entry_readiness_gate_runtime.py --analyze \

#!/usr/bin/env python3
"""Compare Phase 25E MPPI CostCritic compromise experiment against baseline metrics."""
...
def success_efficiency_advisory(base_success: int, exp_success: int, base_goals: int, exp_goals: int) -> dict[str, Any]:
    base_ratio = safe_ratio(base_success, base_goals)
    exp_ratio = safe_ratio(exp_success, exp_goals)
    ratio_delta = None if base_ratio is None or exp_ratio is None else round(exp_ratio - base_ratio, 6)
    return {
        'baseline_goal_count': base_goals,
        'experiment_goal_count': exp_goals,
        'baseline_success_count': base_success,
        'experiment_success_count': exp_success,
        'baseline_success_ratio': base_ratio,
        'experiment_success_ratio': exp_ratio,
        'success_ratio_delta': ratio_delta,
        'raw_success_count_regressed': exp_success < base_success,
    }
...
def compare(baseline: dict[str, Any], experiment: dict[str, Any]) -> dict[str, Any]:
    ...
    base_goals = int(run_value(baseline, 'goal_count'))
    exp_goals = int(run_value(experiment, 'goal_count'))
    ...
    success_gate_passed = exp_success >= base_success
    success_gate_criterion = 'experiment >= baseline unless experiment EXIT_REACHED'
    if exp_mode == 'EXIT_REACHED':
        success_gate_passed = True
        success_gate_criterion = 'experiment EXIT_REACHED bypasses raw success count gate'
    ...
    checks = {
        ...
        'success_gate': check('success_gate', base_success, exp_success, success_gate_passed, success_gate_criterion),
    ...
    advisory = {
        ...
        'success_efficiency': success_efficiency_advisory(base_success, exp_success, base_goals, exp_goals),
    }
    recommendation = 'candidate_baseline_or_repeat_validation'

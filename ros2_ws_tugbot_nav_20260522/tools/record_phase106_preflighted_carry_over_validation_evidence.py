#!/usr/bin/env python3
"""Phase106 raw evidence recorder.

Read-only recorder for Phase105-preflighted carry-over bounded Goal1 staging validation.
Captures /maze/goal_events, /scan, /map, /local_costmap/costmap,
/local_costmap/published_footprint, /odom, and TF. It creates no ActionClient,
...
class Phase106Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase106_preflighted_carry_over_validation_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase106Recorder(args.duration_sec, load_json(args.analysis_json))
    try:
        deadline = time.time() + max(0.1, args.duration_sec)
        while rclpy.ok() and time.time() < deadline:
            rclpy.spin_once(node, timeout_sec=0.1)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(json.dumps(node.snapshot(), indent=2, sort_keys=True) + '\n')
    finally:
        node.destroy_node()
        rclpy.shutdown()
    print(json.dumps({'output': str(args.output), 'run_id': RUN_ID}, sort_keys=True))
    return 0

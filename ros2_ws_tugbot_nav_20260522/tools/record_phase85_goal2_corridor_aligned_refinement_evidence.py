#!/usr/bin/env python3
"""Phase85 raw evidence recorder for bounded Goal2 refinement validation.

Captures a short snapshot of /scan, /local_costmap/costmap,
/local_costmap/published_footprint, /odom and TF at the bounded reproduction
scene. It is observation-only and sends no goals.
"""
...
class Phase85Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase85_goal2_refinement_raw_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase85Recorder(args.duration_sec, source)

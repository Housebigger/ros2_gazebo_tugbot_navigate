#!/usr/bin/env python3
"""Phase102 raw evidence recorder.

Read-only recorder for Phase101 carry-over bounded Goal1 staging validation.
...
class Phase102Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase102_carry_over_validation_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase102Recorder(args.duration_sec, load_json(args.analysis_json))
    try:

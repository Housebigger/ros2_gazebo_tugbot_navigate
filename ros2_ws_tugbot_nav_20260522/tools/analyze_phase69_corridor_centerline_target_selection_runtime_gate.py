...
Guardrails: bounded runtime only, max_goals=1..2, no Nav2/MPPI/controller
...
PHASE68_CLASSIFICATION = 'CENTERLINE_REPLAY_IMPROVES_LOCAL_COST'
...
class Phase69RuntimeRecorder(Node):  # pragma: no cover - requires ROS graph.
    def __init__(self, args: argparse.Namespace) -> None:
        super().__init__('phase69_centerline_runtime_recorder')
        self.args = args
...
def record_runtime(args: argparse.Namespace) -> int:  # pragma: no cover
    ...
    rclpy.init()
    node = Phase69RuntimeRecorder(args)
    try:
...
def summarize_replay(replay_dir: Path, run_id: str = RUN_ID) -> dict[str, Any]:
    ...
    timeline = _read_jsonl(replay_dir / 'phase69_runtime_timeline.jsonl')
    controller_rows = _read_jsonl(replay_dir / 'phase69_controller_dynamics.jsonl')
    feedback_rows = _read_jsonl(replay_dir / 'phase69_nav2_feedback.jsonl')
    local_rows = _read_jsonl(replay_dir / 'phase69_local_costmap_samples.jsonl')
    ...
    return {
        ...
        'centerline_refinement_applied_count': applied_count,
        'centerline_gate_no_apply_count': gate_no_apply_count,
...
def classify_phase69(replays: list[dict[str, Any]], guardrail_violation: bool = False) -> str:
    ...
    if any(r.get('branch_scoring_changed') for r in replays):
        return 'GUARDRAIL_VIOLATION_STRATEGY_CHANGED'
    if any((r.get('centerline_refinement_applied_count') or 0) > 0 and (r.get('timeout_count') or 0) > 0 for r in replays):
        return 'CENTERLINE_RUNTIME_TIMEOUT_REMAINS'
    if any((r.get('centerline_refinement_applied_count') or 0) > 0 and (r.get('dispatch_count') or 0) > 0 for r in replays):
        return 'CENTERLINE_RUNTIME_GOAL1_IMPROVES'
    if any((r.get('centerline_gate_no_apply_count') or 0) > 0 for r in replays):
        return 'CENTERLINE_GATE_NO_APPLY'
    return 'INSUFFICIENT_EVIDENCE'


def analyze_phase69(artifact_dir: Path | str, output: Path | str | None = None) -> dict[str, Any]:
    ...
    max_goals_ok = max_goals is None or 1 <= max_goals <= 2
    ...
    classification = classify_phase69(replays, guardrail_violation)
    summary = {
        'phase': PHASE,
        'run_id': RUN_ID,
        ...
        'phase68_classification_preserved': PHASE68_CLASSIFICATION,
        ...
        'replays': replays,
        'centerline_refinement_applied_count': sum(int(r.get('centerline_refinement_applied_count') or 0) for r in replays),
        'centerline_gate_no_apply_count': sum(int(r.get('centerline_gate_no_apply_count') or 0) for r in replays),
        'metrics': {
        ...
        'complete_autonomous_success_claimed': False,
        'exit_success_claimed': False,
        'recommendations': ['Use Phase69 as bounded Goal1/Goal2 evidence only; stop for human acceptance and do not enter Phase70.'],
    }
...
def analyze(args: argparse.Namespace) -> int:
    summary = analyze_phase69(Path(args.artifact_dir), output=Path(args.output))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--timeline-output', default=str(Path('log') / RUN_ID / 'phase69_runtime_timeline.jsonl'))
    parser.add_argument('--controller-dynamics-output', default=str(Path('log') / RUN_ID / 'phase69_controller_dynamics.jsonl'))
    parser.add_argument('--nav2-feedback-output', default=str(Path('log') / RUN_ID / 'phase69_nav2_feedback.jsonl'))
    parser.add_argument('--local-costmap-samples-output', default=str(Path('log') / RUN_ID / 'phase69_local_costmap_samples.jsonl'))
    parser.add_argument('--global-plan-samples-output', default=str(Path('log') / RUN_ID / 'phase69_global_plan_samples.jsonl'))
    parser.add_argument('--collision-monitor-output', default=str(Path('log') / RUN_ID / 'phase69_collision_monitor_state.jsonl'))

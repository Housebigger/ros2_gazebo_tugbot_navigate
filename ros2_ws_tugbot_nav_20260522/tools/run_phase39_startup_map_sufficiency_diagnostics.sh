...
RUN_TIMEOUT_SEC="${PHASE39_RUN_TIMEOUT_SEC:-150}"
RECORDER_DURATION_SEC="${PHASE39_RECORDER_DURATION_SEC:-95}"
MAX_GOALS="${PHASE39_MAX_GOALS:-1}"
USE_RVIZ="${PHASE39_USE_RVIZ:-false}"
...
SUMMARY_JSON="$ARTIFACT_DIR/phase39_startup_map_sufficiency.json"
PIDS=()
LAUNCH_PID=""
RECORDER_PID=""
...
cleanup() {
  ...
  pkill -TERM -f 'tools/record_phase39_startup_map_sufficiency.py' || true
  ...
  sleep 2
  pkill -KILL -f 'tools/record_phase39_startup_map_sufficiency.py|ros_gz_bridge|parameter_bridge|static_transform_publisher|maze_goal_monitor|maze_explorer|frontier_explorer|slam_toolbox|async_slam_toolbox_node|controller_server|planner_server|bt_navigator|gz sim|ruby .*gz sim|rviz2' || true
  if command -v ros2 >/dev/null 2>&1; then
    ros2 daemon stop >/dev/null 2>&1 || true
  fi
  mkdir -p "$ARTIFACT_DIR"
  pgrep -af 'ros2 launch|gz sim|rviz2|slam_toolbox|maze_explorer|frontier_explorer|controller_server|planner_server|bt_navigator|behavior_server|waypoint_follower|velocity_smoother|smoother_server|ros_gz_bridge|parameter_bridge|static_transform_publisher|maze_goal_monitor|record_phase39_startup_map_sufficiency' \
...
rm -f \
  ...
  "$RECORDER_STDERR" \
  "$SUMMARY_JSON" \
  "$ARTIFACT_DIR/phase39_startup_full_data.json" \
  "$ARTIFACT_DIR/robot_vs_entrance_frame_alignment_overlay.png" \
  "$ARTIFACT_DIR/local_map_known_free_overlay.png" \
  "$ARTIFACT_DIR/topology_sampling_after_full_map_overlay.png" \
...
if (( RUN_TIMEOUT_SEC < 120 || RUN_TIMEOUT_SEC > 180 )); then
  echo "PHASE39_RUN_TIMEOUT_SEC must remain bounded in [120,180], got $RUN_TIMEOUT_SEC" >&2
  exit 7
fi
if (( MAX_GOALS > 1 )); then
  echo "Phase39 startup diagnostics allows at most one bounded analysis/dispatch attempt; set PHASE39_MAX_GOALS=0 or 1 (got $MAX_GOALS)" >&2
  exit 8
fi

{
  echo "# Phase39 Startup Map Sufficiency and Entrance Frame Alignment Evidence"
  ...
  echo "guardrails=diagnostics-only; no Nav2/MPPI/controller parameter edits; no maze_explorer strategy edits; no fallback/terminal acceptance continuation; no old scaffold world/map; no autonomous success claim; no long run; no ros2 topic echo truncated map/costmap evidence."
...
python3 tools/record_phase39_startup_map_sufficiency.py \
  --output-dir "$ARTIFACT_DIR" \
  --duration-sec "$RECORDER_DURATION_SEC" \
  --snapshots 30,60,90 \
  --maze-config "$MAZE_CONFIG" \
  --active-world "$WORLD" \
...
if [[ ! -s "$SUMMARY_JSON" ]]; then
  echo "Phase39 summary missing: $SUMMARY_JSON" >&2
  exit 9
fi
...
print(json.dumps({
    ...
    'failed_exhausted_before_map_sufficient': summary.get('failed_exhausted_before_map_sufficient'),

...
# Guardrails: bounded runtime only; max_goals=3 or 4; no Nav2/MPPI/controller parameter edits;
# no Nav2/MPPI/controller tuning; no inflation/robot_radius/clearance_radius_m/map threshold tuning;
# no branch scoring change; no additional centerline gate relaxation; no corridor-following cmd_vel control;
...
REPLAY_COUNT="${PHASE72_REPLAY_COUNT:-2}"
MAX_GOALS="${PHASE72_MAX_GOALS:-3}"
if (( MAX_GOALS != 3 && MAX_GOALS != 4 )); then
  echo "PHASE72_MAX_GOALS must be 3 or 4; got $MAX_GOALS" >&2
  exit 2
fi
if (( REPLAY_COUNT < 1 || REPLAY_COUNT > 2 )); then
  echo "PHASE72_REPLAY_COUNT must be 1..2 for bounded runtime only; got $REPLAY_COUNT" >&2
  exit 2
fi
...
ARTIFACT_DIR="${PHASE72_ARTIFACT_DIR:-$ROOT/log/${RUN_ID}}"
...
RUN_TIMEOUT_SEC="${PHASE72_RUN_TIMEOUT_SEC:-300}"
READINESS_TIMEOUT_SEC="${PHASE72_READINESS_TIMEOUT_SEC:-80}"
POST_READINESS_SETTLE_SEC="${PHASE72_POST_READINESS_SETTLE_SEC:-10}"
GOAL_TIMEOUT_SEC="${PHASE72_GOAL_TIMEOUT_SEC:-100}"
EXPLORER_OBSERVE_SEC="${PHASE72_EXPLORER_OBSERVE_SEC:-180}"
RUNTIME_RECORD_TIMEOUT_SEC="${PHASE72_RUNTIME_RECORD_TIMEOUT_SEC:-215}"
TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES="${PHASE72_TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES:-2}"
TOPOLOGY_CONSISTENCY_WINDOW_SEC="${PHASE72_TOPOLOGY_CONSISTENCY_WINDOW_SEC:-4.0}"
PIDS=()
LAUNCH_PID=""
STATE_PID=""
GOAL_EVENTS_PID=""
...
cleanup_processes_after() {
  ...
  pkill -TERM -f '[a]nalyze_phase72_multigoal_bounded_rerun_from_inner_ingress.py --send-inner-ingress-goal' || true
  pkill -TERM -f '[a]nalyze_phase72_multigoal_bounded_rerun_from_inner_ingress.py --record-runtime' || true
  pkill -TERM -f '[r]ecord_explorer_state_series.py.*phase72_multigoal_bounded_rerun_from_inner_ingress' || true
  ...
  (pgrep -af '[r]os2 launch|[r]os2 run tugbot_maze maze_explorer|[g]z sim|[r]viz2|[s]lam_toolbox|[m]aze_explorer|[m]aze_goal_monitor|[f]rontier_explorer|[c]ontroller_server|[p]lanner_server|[b]t_navigator|[b]ehavior_server|[w]aypoint_follower|[v]elocity_smoother|[s]moother_server|[r]oute_server|[c]ollision_monitor|[d]ocking_server|[r]os_gz_bridge|[p]arameter_bridge|[s]tatic_transform_publisher|[a]nalyze_phase72_multigoal_bounded_rerun_from_inner_ingress|[r]ecord_explorer_state_series.py.*phase72_multigoal_bounded_rerun_from_inner_ingress' || true) \
    | grep -v 'hermes-snap' \
    | grep -v 'pgrep -af' \
    > "$CLEANUP_AFTER" || true
}
...

{
  ...
  echo "no additional centerline gate relaxation"
  echo "guardrails=bounded runtime only; max_goals=3 or 4; no Nav2/MPPI/controller parameter edits; no Nav2/MPPI/controller tuning; no inflation/robot_radius/clearance_radius_m/map threshold tuning; no branch scoring change; no additional centerline gate relaxation; no corridor-following cmd_vel control; no fallback/terminal acceptance change; no autonomous exploration success claim; no exit success claim."
...
start_runtime_recorder_after_ingress() {
  local replay_dir="$1"
  local replay_id="${replay_dir##*/}"
  python3 tools/analyze_phase72_multigoal_bounded_rerun_from_inner_ingress.py --record-runtime \
    --timeout-sec "$RUNTIME_RECORD_TIMEOUT_SEC" \
    --output "$replay_dir/${RUN_ID}_${replay_id}_runtime_recorder_summary.json" \
    --timeline-output "$replay_dir/phase72_runtime_timeline.jsonl" \
    --controller-dynamics-output "$replay_dir/phase72_controller_dynamics.jsonl" \
    --nav2-feedback-output "$replay_dir/phase72_nav2_feedback.jsonl" \
    --local-costmap-samples-output "$replay_dir/phase72_local_costmap_samples.jsonl" \
    --global-plan-samples-output "$replay_dir/phase72_global_plan_samples.jsonl" \
    --collision-monitor-output "$replay_dir/phase72_collision_monitor_state.jsonl" \
...
for replay_index in $(seq 1 "$REPLAY_COUNT"); do
  ...
  echo "phase72 replay $replay_index/$REPLAY_COUNT starting" >&2
...
JSON
    ...
    python3 tools/analyze_phase72_multigoal_bounded_rerun_from_inner_ingress.py --send-inner-ingress-goal \
...
PY
  ...
  trap cleanup_processes_after EXIT
  sleep 5
  echo "phase72 replay $replay_index/$REPLAY_COUNT completed" >&2
done

cleanup_processes_after
trap - EXIT

python3 tools/analyze_phase72_multigoal_bounded_rerun_from_inner_ingress.py --analyze \

#!/usr/bin/env python3
"""Phase95 terminal evidence recorder.

Captures a short read-only terminal-scene snapshot after the bounded Goal2-
equivalent terminal outcome: latest /maze/goal_events, /scan,
/local_costmap/costmap, /local_costmap/published_footprint, /odom and TF.
It sends no goals and changes no parameters.
"""
...
def scan_to_dict(msg: LaserScan | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        'ranges_min': min((float(v) for v in msg.ranges if math.isfinite(float(v))), default=None),
    }
...
def costmap_to_dict(msg: OccupancyGrid | None) -> dict[str, Any] | None:
    ...
    lethal = sum(1 for v in values if v >= 99)
    high = sum(1 for v in values if v >= 75)
    return {
        ...
        'summary': {'cell_count': len(values), 'lethal_count': lethal, 'high_cost_count': high, 'max': max(values) if values else None},
    }
...
class Phase95Recorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase95_terminal_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase95Recorder(args.duration_sec, load_json(args.source_analysis_json))

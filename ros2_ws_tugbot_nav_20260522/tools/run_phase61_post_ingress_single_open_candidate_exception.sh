...
INGRESS_Y="0.0"
INGRESS_YAW="0.0"
REPLAY_COUNT="${PHASE61_REPLAY_COUNT:-1}"
RUN_TIMEOUT_SEC="${PHASE61_RUN_TIMEOUT_SEC:-300}"
GOAL_TIMEOUT_SEC="${PHASE61_GOAL_TIMEOUT_SEC:-100}"
READINESS_TIMEOUT_SEC="${PHASE61_READINESS_TIMEOUT_SEC:-170}"
EXPLORER_OBSERVE_SEC="${PHASE61_EXPLORER_OBSERVE_SEC:-95}"
STABILITY_RECORD_TIMEOUT_SEC="${PHASE61_STABILITY_RECORD_TIMEOUT_SEC:-120}"
TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES="${PHASE61_TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES:-2}"
TOPOLOGY_CONSISTENCY_WINDOW_SEC="${PHASE61_TOPOLOGY_CONSISTENCY_WINDOW_SEC:-4.0}"
USE_RVIZ="${PHASE61_USE_RVIZ:-false}"
MAX_GOALS="1"

if (( REPLAY_COUNT < 1 || REPLAY_COUNT > 2 )); then
  echo "Phase61 REPLAY_COUNT must be bounded in [1, 2], got $REPLAY_COUNT" >&2
  exit 2
fi
if (( RUN_TIMEOUT_SEC < 150 || RUN_TIMEOUT_SEC > 360 )); then
  echo "Phase61 RUN_TIMEOUT_SEC must be bounded in [150, 360], got $RUN_TIMEOUT_SEC" >&2
  exit 2
fi
if (( EXPLORER_OBSERVE_SEC < 30 || EXPLORER_OBSERVE_SEC > 140 )); then
  echo "Phase61 EXPLORER_OBSERVE_SEC must be bounded in [30, 140], got $EXPLORER_OBSERVE_SEC" >&2
  exit 2
fi
if (( STABILITY_RECORD_TIMEOUT_SEC < 45 || STABILITY_RECORD_TIMEOUT_SEC > 160 )); then
  echo "Phase61 STABILITY_RECORD_TIMEOUT_SEC must be bounded in [45, 160], got $STABILITY_RECORD_TIMEOUT_SEC" >&2
  exit 2
fi
...
cleanup() {
  ...
  pkill -TERM -f '[a]nalyze_phase61_post_ingress_single_open_candidate_exception.py --record-stability' || true
  pkill -TERM -f '[a]nalyze_phase61_post_ingress_single_open_candidate_exception.py --send-ingress-goal' || true
  pkill -TERM -f '[r]ecord_explorer_state_series.py.*phase61_post_ingress_single_open_candidate_exception' || true
  ...
  (pgrep -af '[r]os2 launch|[r]os2 run tugbot_maze maze_explorer|[g]z sim|[r]viz2|[s]lam_toolbox|[m]aze_explorer|[m]aze_goal_monitor|[f]rontier_explorer|[c]ontroller_server|[p]lanner_server|[b]t_navigator|[b]ehavior_server|[w]aypoint_follower|[v]elocity_smoother|[s]moother_server|[r]oute_server|[c]ollision_monitor|[d]ocking_server|[r]os_gz_bridge|[p]arameter_bridge|[s]tatic_transform_publisher|[a]nalyze_phase61_post_ingress_single_open_candidate_exception' || true) \
    | grep -v 'hermes-snap' \
    | grep -v 'pgrep -af' \
    > "$CLEANUP_AFTER" || true
}
...
{
  echo "# Phase61 Post-Ingress Single-Open-Direction Candidate Exception"
  ...
  echo "guardrails=bounded replay only; no Nav2/MPPI/controller parameter edits; no clearance_radius_m tuning; no map sufficiency threshold tuning; no ordinary branch selection scoring change; no fallback/terminal acceptance; no old scaffold world/map; no autonomous exploration success claim; first dispatch is not exit success; does not attribute first-dispatch timeout."
...
start_stability_recorder() {
  local replay_dir="$1"
  local replay_id="${replay_dir##*/}"
  python3 tools/analyze_phase61_post_ingress_single_open_candidate_exception.py --record-stability \
...
for replay_index in $(seq 1 "$REPLAY_COUNT"); do
  ...
  echo "phase61 replay $replay_index/$REPLAY_COUNT starting" >&2
...
{"phase":"Phase61 Post-Ingress Single-Open-Direction Candidate Exception","run_id":"$RUN_ID","replay_id":"$replay_id","goal_sent":false,"success":false,"result_received":false,"reason":"readiness_gates_not_ready"}
JSON
  else
    sleep 3
    {
    ...
    python3 tools/analyze_phase61_post_ingress_single_open_candidate_exception.py --send-ingress-goal \
...
PY
  ...
  trap cleanup EXIT
  sleep 5
  echo "phase61 replay $replay_index/$REPLAY_COUNT completed" >&2
done

cleanup
trap - EXIT

python3 tools/analyze_phase61_post_ingress_single_open_candidate_exception.py --analyze \

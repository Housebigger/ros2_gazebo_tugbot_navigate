...
INGRESS_Y="0.0"
INGRESS_YAW="0.0"
ACCEPTANCE_RADIUS_M="${PHASE56_ACCEPTANCE_RADIUS_M:-0.35}"
RUN_TIMEOUT_SEC="${PHASE56_RUN_TIMEOUT_SEC:-300}"
RECORD_TIMEOUT_SEC="${PHASE56_RECORD_TIMEOUT_SEC:-220}"
GOAL_TIMEOUT_SEC="${PHASE56_GOAL_TIMEOUT_SEC:-100}"
READINESS_TIMEOUT_SEC="${PHASE56_READINESS_TIMEOUT_SEC:-170}"
EXPLORER_OBSERVE_SEC="${PHASE56_EXPLORER_OBSERVE_SEC:-90}"
USE_RVIZ="${PHASE56_USE_RVIZ:-false}"
MAX_GOALS="1"

if (( RUN_TIMEOUT_SEC < 150 || RUN_TIMEOUT_SEC > 360 )); then
  echo "Phase56 RUN_TIMEOUT_SEC must be bounded in [150, 360], got $RUN_TIMEOUT_SEC" >&2
  exit 2
fi
if (( RECORD_TIMEOUT_SEC < 90 || RECORD_TIMEOUT_SEC > 300 )); then
  echo "Phase56 RECORD_TIMEOUT_SEC must be bounded in [90, 300], got $RECORD_TIMEOUT_SEC" >&2
  exit 2
fi
if (( EXPLORER_OBSERVE_SEC < 30 || EXPLORER_OBSERVE_SEC > 140 )); then
  echo "Phase56 EXPLORER_OBSERVE_SEC must be bounded in [30, 140], got $EXPLORER_OBSERVE_SEC" >&2
  exit 2
fi
...
EXPLORER_STATE="$ARTIFACT_DIR/phase56_post_ingress_candidate_formation_diagnostics_explorer_state.jsonl"
GOAL_EVENTS="$ARTIFACT_DIR/phase56_post_ingress_candidate_formation_diagnostics_goal_events.jsonl"
...
OPEN_DIRECTION_DIAGNOSTICS="$ARTIFACT_DIR/open_direction_to_candidate_diagnostics.json"
PIDS=()
LAUNCH_PID=""
RECORDER_PID=""
STATE_PID=""
...
cleanup() {
  ...
  pkill -TERM -f '[a]nalyze_phase56_post_ingress_candidate_formation_diagnostics.py --record-runtime' || true
  pkill -TERM -f '[a]nalyze_phase56_post_ingress_candidate_formation_diagnostics.py --send-goal' || true
  pkill -TERM -f '[r]ecord_explorer_state_series.py.*phase56_post_ingress_candidate_formation_diagnostics' || true
  ...
  (pgrep -af '[r]os2 launch|[r]os2 run tugbot_maze maze_explorer|[g]z sim|[r]viz2|[s]lam_toolbox|[m]aze_explorer|[m]aze_goal_monitor|[f]rontier_explorer|[c]ontroller_server|[p]lanner_server|[b]t_navigator|[b]ehavior_server|[w]aypoint_follower|[v]elocity_smoother|[s]moother_server|[r]oute_server|[c]ollision_monitor|[d]ocking_server|[r]os_gz_bridge|[p]arameter_bridge|[s]tatic_transform_publisher|[a]nalyze_phase56_ingress_then_maze_explorer' || true) \
    | grep -v 'hermes-snap' \
    | grep -v 'pgrep -af' \
    > "$CLEANUP_AFTER" || true
}
...
rm -f \
  ...
  "$FIRST_TOPOLOGY_SNAPSHOT" "$OPEN_DIRECTION_DIAGNOSTICS" \
...
{
  echo "# Phase56 Post-Ingress Open Direction to Candidate Formation Diagnostics"
  ...
  echo "guardrails=bounded post-ingress candidate-formation diagnostics only; no clearance_radius_m tuning; no map threshold tuning; no Nav2/MPPI/controller parameter edits; no fallback/terminal acceptance; no old scaffold world/map; no autonomous exploration success claim; first dispatch is not exit success."
...
start_recorders() {
  python3 tools/analyze_phase56_post_ingress_candidate_formation_diagnostics.py --record-runtime "$RECORD_TIMEOUT_SEC" \
...
{"phase":"Phase56 Post-Ingress Open Direction to Candidate Formation Diagnostics","run_id":"$RUN_ID","goal_sent":false,"success":false,"result_received":false,"reason":"readiness_gates_not_ready"}
JSON
else
  start_recorders
  sleep 3
  ...
  python3 tools/analyze_phase56_post_ingress_candidate_formation_diagnostics.py --send-goal \
...
python3 tools/analyze_phase56_post_ingress_candidate_formation_diagnostics.py --analyze \
  ...
  --first-topology-snapshot "$FIRST_TOPOLOGY_SNAPSHOT" \
  --open-direction-diagnostics "$OPEN_DIRECTION_DIAGNOSTICS" \

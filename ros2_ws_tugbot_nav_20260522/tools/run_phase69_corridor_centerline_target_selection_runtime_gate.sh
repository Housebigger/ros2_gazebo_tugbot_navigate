...
# Phase69 Corridor Centerline Target Selection Runtime Gate / Bounded Smoke
# Guardrails: bounded runtime only; max_goals=1..2; no Nav2/MPPI/controller parameter edits;
# no inflation/robot_radius/clearance_radius_m/map threshold tuning; no branch scoring change;
# no corridor-following cmd_vel control; no fallback/terminal acceptance change; no old scaffold world/map;
...
PHASE="Phase69 Corridor Centerline Target Selection Runtime Gate / Bounded Smoke"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

REPLAY_COUNT="${PHASE69_REPLAY_COUNT:-2}"
MAX_GOALS="${PHASE69_MAX_GOALS:-1}"
if (( MAX_GOALS < 1 || MAX_GOALS > 2 )); then
  echo "PHASE69_MAX_GOALS must be 1..2; got $MAX_GOALS" >&2
  exit 2
fi
if (( REPLAY_COUNT < 1 || REPLAY_COUNT > 3 )); then
  echo "PHASE69_REPLAY_COUNT must be 1..3; got $REPLAY_COUNT" >&2
  exit 2
fi
...
ARTIFACT_DIR="${PHASE69_ARTIFACT_DIR:-$ROOT/log/${RUN_ID}}"
...
RUN_TIMEOUT_SEC="${PHASE69_RUN_TIMEOUT_SEC:-240}"
READINESS_TIMEOUT_SEC="${PHASE69_READINESS_TIMEOUT_SEC:-80}"
POST_READINESS_SETTLE_SEC="${PHASE69_POST_READINESS_SETTLE_SEC:-10}"
GOAL_TIMEOUT_SEC="${PHASE69_GOAL_TIMEOUT_SEC:-100}"
EXPLORER_OBSERVE_SEC="${PHASE69_EXPLORER_OBSERVE_SEC:-120}"
RUNTIME_RECORD_TIMEOUT_SEC="${PHASE69_RUNTIME_RECORD_TIMEOUT_SEC:-145}"
TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES="${PHASE69_TOPOLOGY_CONSISTENCY_REQUIRED_NO_CANDIDATE_FRAMES:-2}"
TOPOLOGY_CONSISTENCY_WINDOW_SEC="${PHASE69_TOPOLOGY_CONSISTENCY_WINDOW_SEC:-4.0}"
PIDS=()
LAUNCH_PID=""
STATE_PID=""
GOAL_EVENTS_PID=""
...
cleanup_processes_after() {
  ...
  pkill -TERM -f '[a]nalyze_phase69_corridor_centerline_target_selection_runtime_gate.py --send-inner-ingress-goal' || true
  pkill -TERM -f '[a]nalyze_phase69_corridor_centerline_target_selection_runtime_gate.py --record-runtime' || true
  pkill -TERM -f '[r]ecord_explorer_state_series.py.*phase69_corridor_centerline_target_selection_runtime_gate' || true
  ...
  (pgrep -af '[r]os2 launch|[r]os2 run tugbot_maze maze_explorer|[g]z sim|[r]viz2|[s]lam_toolbox|[m]aze_explorer|[m]aze_goal_monitor|[f]rontier_explorer|[c]ontroller_server|[p]lanner_server|[b]t_navigator|[b]ehavior_server|[w]aypoint_follower|[v]elocity_smoother|[s]moother_server|[r]oute_server|[c]ollision_monitor|[d]ocking_server|[r]os_gz_bridge|[p]arameter_bridge|[s]tatic_transform_publisher|[a]nalyze_phase69_corridor_centerline_target_selection_runtime_gate|[r]ecord_explorer_state_series.py.*phase69_corridor_centerline_target_selection_runtime_gate' || true) \
    | grep -v 'hermes-snap' \
    | grep -v 'pgrep -af' \
    > "$CLEANUP_AFTER" || true
}
...

{
  ...
  echo "phase68_classification=CENTERLINE_REPLAY_IMPROVES_LOCAL_COST"
  ...
  echo "guardrails=bounded runtime only; max_goals=1..2; no Nav2/MPPI/controller parameter edits; no inflation/robot_radius/clearance_radius_m/map threshold tuning; no branch scoring change; no corridor-following cmd_vel control; no fallback/terminal acceptance change; no old scaffold world/map; no autonomous exploration success claim; no exit success claim."
  echo "maze_explorer_command=ros2 run tugbot_maze maze_explorer --ros-args -p max_goals:=$MAX_GOALS -p near_exit_fallback_enabled:=false -p centerline_target_refinement_enabled:=true"
...
start_runtime_recorder_after_ingress() {
  local replay_dir="$1"
  local replay_id="${replay_dir##*/}"
  python3 tools/analyze_phase69_corridor_centerline_target_selection_runtime_gate.py --record-runtime \
    --timeout-sec "$RUNTIME_RECORD_TIMEOUT_SEC" \
    --output "$replay_dir/${RUN_ID}_${replay_id}_runtime_recorder_summary.json" \
    --timeline-output "$replay_dir/phase69_runtime_timeline.jsonl" \
    --controller-dynamics-output "$replay_dir/phase69_controller_dynamics.jsonl" \
    --nav2-feedback-output "$replay_dir/phase69_nav2_feedback.jsonl" \
    --local-costmap-samples-output "$replay_dir/phase69_local_costmap_samples.jsonl" \
    --global-plan-samples-output "$replay_dir/phase69_global_plan_samples.jsonl" \
    --collision-monitor-output "$replay_dir/phase69_collision_monitor_state.jsonl" \
...
for replay_index in $(seq 1 "$REPLAY_COUNT"); do
  ...
  echo "phase69 replay $replay_index/$REPLAY_COUNT starting" >&2
...
JSON
    ...
    python3 tools/analyze_phase69_corridor_centerline_target_selection_runtime_gate.py --send-inner-ingress-goal \
...
PY
  ...
  trap cleanup_processes_after EXIT
  sleep 5
  echo "phase69 replay $replay_index/$REPLAY_COUNT completed" >&2
done

cleanup_processes_after
trap - EXIT

python3 tools/analyze_phase69_corridor_centerline_target_selection_runtime_gate.py --analyze \

#!/usr/bin/env python3
"""Phase96-fix raw evidence recorder.

Read-only recorder for ingress-guided Phase96 smoke. Captures /maze/goal_events,
/scan, /map, /local_costmap/costmap, /local_costmap/published_footprint,
/odom, and TF. It creates no action client and changes no parameters.
"""
...
class Phase96FixRecorder(Node):
    def __init__(self, duration: float, source_analysis: dict[str, Any]) -> None:
        super().__init__('phase96_fix_smoke_evidence_recorder')
...
def main() -> int:
    ...
    rclpy.init()
    node = Phase96FixRecorder(args.duration_sec, load_json(args.analysis_json))
    try:
        ...
        print(json.dumps({'output': str(args.output), 'scan': snapshot['scan'] is not None, 'map': snapshot['map'] is not None, 'odom': snapshot['odom'] is not None, 'tf_keys': sorted(snapshot['tf'])}, sort_keys=True))
    finally:
        node.destroy_node()
        rclpy.shutdown()
    return 0

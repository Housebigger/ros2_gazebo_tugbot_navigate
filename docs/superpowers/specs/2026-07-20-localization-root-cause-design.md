...
4. **统计门 + 门动作从"封顶"升级为"恢复"(2026-07-21/22,用户裁决 A)。**
   ...
   - **+3 加固批(N=8 总计)**:3/3 EXIT、全 clean、oracle 0.000%、0 次触发(未进
     混叠区)。恢复门总计 **8/8 EXIT、oracle 全 0.000%、无一混叠锁定**;2/8 跑触发门
     且都干净恢复。诚实注:触发样本仅 2 例(混叠 ~1/3 随机 + 恢复后不复触发使触发跑
     变稀有),但对照封顶批 3 触发跑全带残差蹭墙,机制反证充分(触发次数 25→2 的对比
     是核心证据)。
   - **结论**:odom 先验 yaw 门(恢复式)消除栅格混叠 P1 失败——混叠锁定归零、
     残差蹭墙归零、通关率 4/8→5/5。计划两杠杆(locality gate/rms 天花板)被单一
     对症杠杆替代。

5. **GUI 验收(2026-07-22,用户画面确认通过)**:`20260722_083834` = EXIT_REACHED,
   oracle 官方 0.000% / live_rate 0.000%(168 采样零碰撞),gt-solver |yaw| p90=0.10 /
   max=0.48、pinned>0.8=0%(无混叠锁定)。此跑**恰触发门 3 次**——机器人过混叠区、
   ICP 想偏、门 snap yaw 到 odom 即刻恢复(|yaw| 够 0.48 即拉回、无残差),零碰撞:
   设计意图的现场演示。用户画面确认:`/maze/self_built_map` 雷达累积图全程与实时
   3D 点云贴合,触发时短暂微动即回正(对照上阶段"越漂越远锁死 90°"的错位视觉症状
   已消失)。阶段按用户裁决进入终审 + 合并收尾。

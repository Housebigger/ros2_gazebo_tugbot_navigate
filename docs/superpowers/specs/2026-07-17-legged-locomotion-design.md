...
14. **终审补漏(五项 spec-body 偏差,前 13 条未覆盖)**:①`/locomotion/state` 诊断话题第 5.2 节承诺但未建——由 1 Hz LOCO 日志行等价替代(wrapper 直接 grep launch.log,少一条话题链路);②第 7 节"DIAG 增加 roll/pitch"落在 LOCO 日志行而非 solver 的 DIAG 行(solver 冻结不动,姿态数据同样落盘);③第 5.1 节"解析 IK"实际落地为数值阻尼 Gauss-Newton(避免四腿镜像手推代数的符号错误风险;FK 锚定 SDF,正确性由 FK-对-SDF 锁定测试与往返测试保证);④第 5.2 节 ω_max 由 spec 的 0.7 收紧为 wz_max=0.5(与 solver w_max 默认 0.5 精确对齐,第 6 节包络推导同步);⑤第 8 节阶梯站立档 z 上界 0.56→0.60(实测站高 0.519,带余量;下界 0.42 不变)。
15. **GUI 验收环境要求:必须 PRIME offload 到独显(A/B 实锤)**。本机 `:1` 是跑在 AMD Radeon 610M 核显(2CU)上的物理 Xorg;GUI 模式下 Gazebo 窗口 + `gpu_lidar` 传感器渲染与 GNOME 合成器同挤这块弱核显,RTX 4070 完全闲置。后果:GUI 跑#1(带 RViz)EXIT 但 oracle **6.944%**(10/144,全部集中在 ICP 弱区门洞格线上,姿态 ≤1.4°、0 STALL 物理接触反证成立 → 判定为 ICP 位姿错位污染 oracle 输入,非真实碰撞);GUI 跑#2(无 RViz)ICP 劣化加剧,在 (6,5)↔(7,5) 活锁(76 次 STALL、12 次 escape),1613 s 后人工终止。修复 = `__NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia` 把渲染整体推上 RTX 4070。修复后 GUI 跑#3(无 RViz)与 #4(带 RViz)均 EXIT_REACHED(~706/694 s)、oracle **0.000%**、ICP rms 中位 0.026/0.034——与 headless 完全同质。**结论:headless 验证不受影响;GUI 运行(含 RViz)必须带 PRIME offload 环境变量**,README 的 GUI 命令已注明。
16. **探照灯穿墙修复(用户验收反馈,视觉层)**:CERBERUS 原装 4 个聚光灯(up/forward/left/right)无 `cast_shadows` 且迷宫世界 `<scene><shadows>false</shadows>` 全局关阴影,光柱直穿墙体。修复 = 4 灯 `cast_shadows=true` + 世界 scene shadows=true(太阳光保持无阴影,只有聚光灯付阴影贴图代价,PRIME offload 后开销可忽略)。gpu_lidar 只测深度不感知光照,导航/物理零影响;站立冒烟复跑通过(z=0.522)。

...
5. **统计门首跑抓出 P1 尾巴:纯位置 ±1 格混叠(yaw 干净)**。取证链:2 个 oracle 样本 = **信念位姿穿真实静态墙**(世界墙盒与 oracle 段逐坐标对账一致,57 模型全 static 不可推)+ odomcell 领先 dcell;POSEDIAG 复跑(6287 三路样本)钉死:gt-odom≈0(odom 仍真值)、|yaw| p90=0.03(c52e951 yaw 门有效)、|odom−solver| 位置**完美双峰**(清洁 ~4233 样本 p99 0.182(终审复算:0.822 属带间散点,清洁主体 max 0.410;±11 样本桶边界出入不影响分布结论) vs 混叠 2052 样本钉 ~2.0,0.5-1.5 带仅 2 样本),**单窗锁死至跑终**(EXIT 在混叠态声明,修前两跑作废)。物理车慢速停走流重新暴露了 yaw 门结构性看不见的位置半边。
...
8. **Task 5c:转弯行为优化(用户提案,SHA 658b4b7+7d1bdda)**:GUI 取证证实转弯是刷厚热点(15 个体素抬升段 11 个贴转弯相)后,用户提出"先倒车、一次过弯"。落地为两原语:①**reverse-hop 链**(回撤=直线倒退,复用 P3 backout 机械,Trémaux 记账镜像、不计死端预算、超时按 (cell,target) 封禁回退 N 点)——180° 掉头基本消灭;②**back-and-arc**(直退 d_back=min(0.65, 尾向 perp−0.30)≥0.45,单段前进四分之一弧 R=d_back 切双中心线,曲率 ≤2.22 舵限内)。N 点降为兜底。审查抓出弧后 hop_start 重锚未抵扣 d_back 净前进(每路口冲过 0.6m、边缘落墙)→ 修为弧后保留路由时锚(7d1bdda);flag 生命周期枚举无泄漏。离线全解 758.5→598.9s。
9. **统计门复验(5c 后两跑)**:均 EXIT、oracle **0.000%**(122/122)、零 ERROR、**双混叠门零触发**;体素 **664,737 / 674,185**——从 5c 前的 728/745k 回落**到运动学基线水平**(649-711k),转弯刷厚源被量化消除;通关 ~605s(比 5c 前 headless 的 630/632s 快 ~4.3%——终审复算纠正:初记 ~11% 误用了 671s 的 GUI 取证跑作基线;另录:5c 反使离线全解变慢 534→599s 而真跑变快,离线 sim 不含转弯物理成本)。工件:`log/flood_fill_run_20260723_094123`、`_095224`。
10. **Task 5d:后半段鬼影根除(用户报症,SHA bc8fbe6)**:用户报"后半段 3D 图交叠依然严重"。诊断链:①**饱和悖论**——GUI 跑后 3/5 体素增速恒为 headless 的 2 倍(415k/350k/199k vs 199k/181k/93k,+49 万鬼影),headless 两跑互相吻合;②**ICP 无罪**——后 60% 定位健康度三跑不可区分(拒配/rms 同分布),双门零触发;③机制 = 累积器**最新 TF 配积压旧点云**(导出随图变大变慢 ~300-450ms → 回调阻塞 → 队列积压 ~0.5s → 旧云配新位姿 0.1-0.2m 壳层,正反馈,GUI 渲染争抢放大)——正是 cloud-map-3d spec 预档的已接受风险与预置后手。修法 = **按 cloud.header.stamp 查 TF**(tf2 历史)+ 帧龄 >0.5s 直接丢弃。
11. **验证与意外红利 + GUI 验收通过(2026-07-23,用户)**:修后 headless 454,733(stale_dropped 9)——比修前 665/674k **再降 32%**:latest-TF 错位在 headless 也一直潜伏(4-12cm 底噪层),一并剥除;**GUI 复跑 449,948(−62%),与 headless 差归零**,饱和曲线回归衰减形(140k/101k/53k),EXIT/oracle 0.000%/零 ERROR/双门零触发。**本迷宫 0.05m 体素的真实干净图 ≈ 45 万 = 狗基线的 ~18%**;此前一切基线(含运动学档)均含该底噪。用户画面确认后半段交叠消失,验收通过。工件:`log/flood_fill_run_20260723_115121`(headless 对照)、`_133656`(GUI 判决跑)、`_112954`(修前 GUI 取证)。
